...
def create_forecasting_complication_graph():
    ...
    layout = go.Layout(
        ...
        margin=dict(l=250, r=40, t=40, b=40, pad=10),
        titlefont={
...
def create_forecasting_cause_fatal_outcome_graph():
    ...
    layout = go.Layout(
        ...
        height=500,
        font={"family": "Helvetica Neue, sans-serif", "size": 12},
        margin=dict(l=350, r=40, t=40, b=40, pad=10),
        titlefont={
            "family": "Helvetica, sans-serif",
            "size": 26,
        },
...
@app.callback(
    ...
    Input("id_add_patient_btn", "n_clicks")],
    State("id_complication_graph", "figure"),
    prevent_initial_call=True
)
def forecasting_complication(btn1, btn2, figure):
    triggered_id = ctx.triggered_id
    if triggered_id == 'id_forecasting_complication_btn':
        figure["data"] = [go.Bar(
                y=["Фибрилляция предсердий", "Суправентрикулярная тахикардия", "Желудочковая тахикардия"
                    , "Фибрилляция желудочков", "Полная АВ блокада", "Отек легких", "Разрыв сердца"
                    , "Синдром Дресслера", "Хроническая сердечная недостаточность", "Рецидив инфаркта миокарда"
        ...
        return figure
    elif triggered_id == 'id_add_patient_btn':
        return create_forecasting_complication_graph()
...
def forecasting_fatal_outcome(btn1, btn2, pie, bar):
    triggered_id = ctx.triggered_id
    if triggered_id == 'id_forecasting_fatal_outcome_btn':
        pie["data"] = [go.Pie(
            labels=["Выжил", "Умер"],
        ...
        return create_forecasting_fatal_outcome_graph(), create_forecasting_cause_fatal_outcome_graph()
...
def entry_patient_into_df(all_inputs):
    if all_inputs['button']:
        ...
        return table, build_patients_info_layout()
    raise PreventUpdate


@app.callback(
    Output("hidden_div", "children"),
    Input('id_table_patients', 'active_cell'),
    prevent_initial_call = True
)
def table_select(cell_index):
    global previous_row
    global current_index
    if cell_index and cell_index['row'] != previous_row:
        current_index = cell_index['row']
        previous_row = current_index
        return []
    raise PreventUpdate
